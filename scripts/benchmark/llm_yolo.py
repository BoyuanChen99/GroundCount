...
# LLM
args.add_argument("--model", type=str, default="Qwen/Qwen3-0.6B", help="Model name")
...
args.add_argument("--temperature", type=float, default=0.7, help="Sampling temperature. When set to 0.0 the model will do greedy decoding and ignore the other parameters. ")
...
def main(args):
    ...
    for subset in all_subsets:
        ...
        if args.provide_yolo_info:
            output_model_dir = os.path.join(output_data_dir, f"{model_name_short}_yolo_only")
        ...
        ### Step 3: Loop through each question in the dataframe
        for idx, row in tqdm(df.iterrows(), total=len(df)):
            ...
            ### Step 6: Feed into the LLM for processing and question answering
            response, num_generated_tokens, elapsed_time = llm.infer_with_stats(
                    prompt=prompt, 
